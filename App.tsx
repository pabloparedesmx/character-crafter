...
const CARTOON_STYLES = {
    'Classic Animation': [
        { id: 'vintage-cartoon', name: 'Vintage Cartoon Style', description: 'Bold outlines with simple shapes and high contrast' },
        { id: 'traditional-cel', name: 'Traditional Cel Animation', description: 'Smooth lines with classic animation proportions' },
        { id: 'rubber-hose', name: 'Rubber Hose Animation', description: 'Fluid, stretchy limbs with simple rounded forms' },
    ],
    'Modern Illustration': [
        { id: 'contemporary-flat', name: 'Contemporary Flat Design', description: 'Clean vector-style with bold colors' },
        { id: 'semi-realistic', name: 'Semi-Realistic Cartoon', description: 'Detailed features with cartoon proportions' },
        { id: 'minimalist-modern', name: 'Minimalist Modern', description: 'Simplified forms with geometric shapes' },
    ],
    'Manga & Anime Inspired': [
        { id: 'manga-style', name: 'Manga Illustration', description: 'Sharp lineart with dramatic expressions' },
        { id: 'anime-style', name: 'Anime Character Design', description: 'Large eyes with detailed hair and shading' },
        { id: 'chibi-kawaii', name: 'Chibi Style', description: 'Super cute with oversized heads and small bodies' },
        { id: 'shounen-action', name: 'Action Anime Style', description: 'Dynamic poses with spiky hair and bold expressions' },
        { id: 'soft-anime', name: 'Soft Anime Style', description: 'Gentle colors with whimsical fantasy elements' },
    ],
    'Animation Techniques': [
        { id: 'cutout-animation', name: 'Cutout Animation Style', description: 'Flat layered shapes with simple joints' },
        { id: 'geometric-cartoon', name: 'Geometric Cartoon', description: 'Bold shapes with angular design elements' },
        { id: 'organic-shapes', name: 'Organic Shape Animation', description: 'Flowing curves with natural movement feel' },
        { id: 'vector-illustration', name: 'Vector Illustration', description: 'Clean digital art with smooth gradients' },
    ],
    'Artistic Approaches': [
        { id: 'gothic-cartoon', name: 'Gothic Cartoon Style', description: 'Dark themes with elongated proportions' },
        { id: 'comic-strip', name: 'Comic Strip Style', description: 'Expressive linework with classic comic proportions' },
        { id: 'watercolor-cartoon', name: 'Watercolor Cartoon', description: 'Soft painted textures with flowing colors' },
        { id: 'sketch-animation', name: 'Sketch Animation Style', description: 'Rough, hand-drawn aesthetic with visible strokes' },
    ],
};
...
type AppState = 'idle' | 'image-uploaded' | 'style-selection' | 'generating-base' | 'approval' | 'generating-sheet' | 'results-shown';
...
function App() {
    ...
    const getStylePrompt = (styleId: string): string => {
        ...
        const stylePrompts: Record<string, string> = {
            'vintage-cartoon': 'vintage cartoon style with bold black outlines, simple shapes, and high contrast colors',
            'traditional-cel': 'traditional cel animation style with smooth clean lines, classic proportions, and vibrant colors',
            'rubber-hose': 'rubber hose animation style with fluid stretchy limbs, rounded forms, and bouncy characteristics',
            'contemporary-flat': 'contemporary flat design style with clean vector graphics, bold solid colors, and minimal shading',
            'semi-realistic': 'semi-realistic cartoon style with detailed facial features, natural proportions, and subtle shading',
    ...
    React.useEffect(() => {
        ...
        allStyleIds.forEach(styleId => {
            examples[styleId] = `/style-examples/${styleId}.png`;
        });
...
FINAL REMINDER: This must be the SAME CHARACTER as shown in the reference images - identical in every visual aspect except for the pose.`;
    ...
    const handleDownloadSheet = async () => {
        ...
        try {
            ...
            const sheetDataUrl = await createCharacterSheet(imageData);
    ...
    const isGenerationComplete = appState === 'results-shown' && Object.values(generatedImages).every(img => img.status === 'done' || img.status === 'error');
                        ...
                        >
                            ...
                            {selectedStyle && (
                                <div className="w-full space-y-6">
                                    <h3 className="font-permanent-marker text-2xl text-neutral-200 text-center">Choose Art Style</h3>
                                    {Object.entries(CARTOON_STYLES).map(([category, styles]) => (
